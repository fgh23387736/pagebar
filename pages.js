...
mpp.init({
	...
	atherdata:",,",//用户需要传到后台的多余数据可以为字符串，数字，或者json数据(此项可用来筛选数据，做查询功能)
	pgz:1,//每页显示数据条数
...
/*	function GetSchoolList(){
		...
		$pg=$_POST['page'];//不用修改
		$atherdata=$_POST['atherdata'];//不用修改
		$atherdataarr = explode(",", $atherdata);//按照逗号分隔，若前台传过来的是json数据则另行处理
		$data=array();//不用修改
		$sql_num="select * from school";//输入需要字段和表名

		//以下是筛选条件
		$sql_num.=" where Name like '%".$atherdataarr[0]."%'";
		if(isset($atherdataarr[1])&&$atherdataarr[1]!=""){
			$sql_num.=" and CityId=".$atherdataarr[1];
		}
		if(isset($atherdataarr[2])&&$atherdataarr[2]!=""){
			$sql_num.=" and CityId in (select Id from city where ProvinceId=".$atherdataarr[2].")";
		}
		//筛选条件结束
		...
		init:function(p){
			...
			$.each(p,function(name,value) {
				switch(name){
					...
					break;
					case 'atherdata':that.atherdata=value;
					break;
